...
class ComicTextDetector(OfflineDetector):
    ...
    async def _infer(self, image: np.ndarray, detect_size: int, text_threshold: float, box_threshold: float,
        ...
        # NOTE: detect_size is intentionally NOT used to resize the model's input here. This
        # model was trained at a fixed self.input_size (1024x1024) and is not resolution-
        # adaptive in practice: box coordinates still map back to the original image correctly
        # at any tile size (SegDetectorRepresenter scales proportionally), but empirically the
        # detector/OCR quality measurably degrades once the tile size departs from what it was
        # trained on (confirmed: 1024 reads text cleanly, 1536 drops a region, 2560 produces
        # garbled OCR on identical input). Use box_threshold (actually wired below, unlike
        # before) to tune recall instead; for genuinely tiny/corner-panel text, use per-request
        # config.upscale.upscale_ratio to upscale the source image before detection instead.
        tgt_size = self.input_size[0]
        lines_map, mask = det_rearrange_forward(image, self.det_batch_forward_ctd, tgt_size, 4, self.device, verbose)
        # blks = []
        # resize_ratio = [1, 1]

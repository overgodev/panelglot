...
class CustomOpenAiTranslator(ConfigGPT, CommonTranslator):
    ...
    async def _translate(self, from_lang: str, to_lang: str, queries: List[str]) -> List[str]:
        ...
        # `_assemble_prompts` splits `queries` into several requests whenever the combined
        # text is too long for one prompt (see `_MAX_TOKENS`) - e.g. a whole story OCR'd and
        # queued as a single translate() call still ends up as multiple sequential requests.
        # Without this, each request after the first started from a blank slate and had no
        # memory of what was translated a few requests ago, which looked like the model
        # "forgetting" everything past the first few pages. Carry the previous request's
        # queries/translations forward as extra reference context for the next one.
        base_prev_context = self.prev_context
        carried_context = base_prev_context
        chunk_start = 0
        ...
        try:
            for prompt, query_size in self._assemble_prompts(from_lang, to_lang, queries):
                ...
                self.prev_context = carried_context
                ...
                # Build reference context for the *next* chunk out of this chunk's
                # query/translation pairs, chained after whatever context we started with.
                chunk_queries = queries[chunk_start:chunk_start + query_size]
                chunk_start += query_size
                pairs = [
                    f"<|{i + 1}|>{q.strip()} -> {t}"
                    for i, (q, t) in enumerate(zip(chunk_queries, new_translations))
                    if q.strip() and t
                ]
                if pairs:
                    chunk_block = "Here are the immediately preceding translations for reference:\n" + "\n".join(pairs)
                    carried_context = f"{base_prev_context}\n\n{chunk_block}" if base_prev_context else chunk_block
        finally:
            self.prev_context = base_prev_context
    ...
    async def _request_translation(self, to_lang: str, prompt: str) -> str:
        ...
        # Give the model recent page(s) of translation history as extra context, if configured
        # (see Config.translator context_size / manga_translator._dispatch_with_context).

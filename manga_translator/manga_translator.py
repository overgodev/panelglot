...
from .translators.common import ISO_639_1_TO_VALID_LANGUAGES, VALID_LANGUAGES
...
class MangaTranslator:
    ...
    def _set_image_context(self, config: Config, image=None):
        ...
        # Persist the originally uploaded filename (if the caller provided one) next to the
        # result, so callers like the web UI's "download all" can restore it on export.
    ...
    async def _run_detection(self, config: Config, ctx: Context):
        ...
        # Fallback: if the configured detector found nothing, retry once with ctd before
        # giving up. The two use different underlying models/architectures, so a page that
        # trips up one (e.g. busy/textured backgrounds) can still be caught by the other.
        textlines = result[0] if result else None
        if not textlines and config.detector.detector != Detector.ctd:
            logger.info(f'{config.detector.detector} detector found no text regions, retrying with ctd as fallback')
            self._model_usage_timestamps[("detection", Detector.ctd)] = current_time
            fallback_result = await dispatch_detection(Detector.ctd, ctx.img_rgb, config.detector.detection_size, config.detector.text_threshold,
                                            config.detector.box_threshold,
                                            config.detector.unclip_ratio, config.detector.det_invert, config.detector.det_gamma_correct, config.detector.det_rotate,
                                            config.detector.det_auto_rotate,
                                            self.device, self.verbose)
            if fallback_result and fallback_result[0]:
                logger.info('ctd fallback found text regions the primary detector missed')
